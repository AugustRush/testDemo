         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.section) {
        ...
        case BuyRyFitMainSectionTypePaybox:
        {
            ...
            for (int i = 0; i < _productList.count; i++) {
                BR_ProductEntity *_p = _productList[i];
                _tPrice += ( [_p.pt_price floatValue] * _p.pt_countOfBuy );
            }
            
            [_cellT updateCell:@{ @"p":[NSString stringWithFormat:@"%.2f",_tPrice] }
                         index:indexPath];
            return _cellT;
        }
            break;
    ...
    if ( _tpF == 0) {
        switch (_tp) {
            ...
            case 1:
            {
                ...
                _prVc.tp        = 1;
                _prVc.tradeNo   = _orderNo;
                _prVc.url       = [NSString stringWithFormat:@"%@?username=%@&password=%@&orderSn=%@",
                                   kGetTradeInfo_URL,
                                   _username,
                                   _password,
                                   _orderNo];
    ...
    [[InterfaceModel sharedInstance] getOrderWithCallback:
     ^(WebCallBackResult result, id successParam, NSString *errorMsg) {
         ...
         if (result == WebCallBackResultSuccess) {
             ...
             if (!successParam ||
                 ![successParam isKindOfClass:[NSDictionary class]])
             {
                 [self showHUDInWindowJustWithText:@"数据异常,获取订单失败" disMissAfterDelay:0.8];
                 return;
             }
             ...
             _orderNo           = [Helpers inputStr:_dic[@"out_trade_no"]];
             _password          = [Helpers inputStr:successParam[@"password"]];
             _username          = [Helpers inputStr:successParam[@"username"]];
             ...
             _tempDic           = @{ @"orderNo":_dic[@"out_trade_no"],
                                    ...
                                    @"userInfo":@{
                                        ...
                                        @"password":_password,
                                        @"username":_username,
                                        @"pid":[NSString stringWithFormat:@"%d",_pareantId]
                                         }
                                };
