...
- (void)getDeviceColor {
    ...
    [_interfaceService getMACAddressCompletion:^(int code, id info, id originalData) {
        ...
        NSString *macInfo = [(NSString *)info uppercaseString];
        NSArray *infoA = [macInfo componentsSeparatedByString:@":"];
        NSMutableString *lastMacStr = [NSMutableString string];
        [infoA enumerateObjectsUsingBlock:^(NSString* obj, NSUInteger idx, BOOL *stop) {
            [lastMacStr appendString:obj];
        }];
        NSLog(@"last mac string is %@",lastMacStr);
        [[InterfaceModel sharedInstance] getDevColorWithMac:lastMacStr callback:^(WebCallBackResult result, DevColor color, NSString *errorMsg) {
            NSLog(@"获取到设备颜色:%d", color);
            if ([[NSUserDefaults standardUserDefaults] objectForKey:@"_hasChangeSystemStyle"]) {
                return ;
            }
            
            switch (color) {
                case DevColorFaile:
//                    [[ThemeManager sharedManager] changeToTheme:ThemeStyleClassic];
                    break;
                case DevColorWhite:
                    [[ThemeManager sharedManager] changeToTheme:ThemeStyleClassic];
                    break;
                case DevColorBlack:
                    ...
                    break;
                case DevColorRed:
                    [[ThemeManager sharedManager] changeToTheme:ThemeStyleRed];
                    break;
                case DevColorGreen:
                    [[ThemeManager sharedManager] changeToTheme:ThemeStyleGreen];
                    break;
                case DevColorYellow:
                    [[ThemeManager sharedManager] changeToTheme:ThemeStyleYellow];
                    break;
