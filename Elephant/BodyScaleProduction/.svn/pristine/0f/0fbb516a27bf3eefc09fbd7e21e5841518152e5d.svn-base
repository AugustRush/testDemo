...
- (id)init
{
    ...
    {
        //腾讯微博注册
//        tcWeiboEngine = [[TCWBEngine alloc] initWithAppKey:kTcAppKey andSecret:kTcAppSecret andRedirectUrl:kTcRedirectURI];
...
{
    self.delegate = nil;
//    if (nil != tcWeiboEngine)
//    {
//        [tcWeiboEngine release];
//        tcWeiboEngine = nil;
//    }
...
- (BOOL)handleOpenURL:(NSURL *)url
{
    ...
    else if([url.absoluteString hasPrefix:@"aliyPay19841111"])
    {

        //NSString *_str = [Helpers URLDecodedString:[url absoluteString]];


            [[NSNotificationCenter defaultCenter]
                      postNotificationName:kAlipayNotificationName
                                    object:nil
                                  userInfo:@{@"urlStr":[url absoluteString]}];
...
//        //[self sendAppPictureWithImage:<#(UIImage *)#> WithUrl:<#(NSString *)#> WithScene:<#(int)#>]
//        return;
//    }
...
- (void)sendWeChatMessageWithType:(ShareType)weiboType sendImage:(UIImage *)sendImage
{
    ...
    {
        //[self sendAppContentWithMessage:message WithUrl:url WithScene:WXSceneSession];
        [self sendImageContentWithScene:WXSceneSession sendImage:sendImage];
        NSLog(@"%@",sendImage);
        return;
    }
    else if(weChatFriend == weiboType)
    {
        //[self sendAppContentWithMessage:message WithUrl:url WithScene:WXSceneTimeline];
        [self sendImageContentWithScene:WXSceneTimeline sendImage:sendImage];
        return;
    }
}


//- (void)sendShareMessage:(NSString*)message WithType:(ShareType)weiboType
//{
...
//        
//        [self sinaWeiboPostImage:[UIImage imageNamed:@"120.png"] WithStatus:message];
//    }
////    else if(tcWeibo == weiboType)
////    {
...
- (void)loginSuccess:(ShareType)weibotype
{
...
- (void)logOutSuccess:(ShareType)weibotype
{
...
- (void)weChatPostStatus:(WXMediaMessage*)message
{
...
- (void)weChatFriendPostStatus:(WXMediaMessage*)message
{
    SendMessageToWXReq* req = [[[SendMessageToWXReq alloc] init]autorelease];
    req.bText = NO;
    req.message = message;
...
- (void) sendImageContentWithScene:(int)scene sendImage:(UIImage *)image
{
    WXMediaMessage *message = [WXMediaMessage message];
