...
static const NSInteger kYearCount = 80;
...
- (void)initViews {
    ...
    NSInteger selectedYear = (self.years.count + kminAge) - self.initialAge;
    if (selectedYear < self.years.count) {
        [self.pickerView selectRow:selectedYear inComponent:0 animated:NO];
    }
    
    NSInteger selectedMonth = _initialMonth - 1;
    if (selectedMonth < self.months.count) {
        [self.pickerView selectRow:selectedMonth inComponent:1 animated:NO];
    }
    
    NSInteger selectedDay = _initialDay - 1;
    if (selectedDay < self.days.count) {
        [self.pickerView selectRow:selectedDay inComponent:2 animated:NO];
    }
}
    ...
    switch (self.pickerMode) {
        case AQPickerModeYearMonthDay: {
            ...
            if ([self.delegate respondsToSelector:
                            @selector(pickerViewDidStopWithYear:month:day:)]
            ) {
                NSInteger year = [self.years[selectedYear] integerValue];
                NSInteger month = [self.months[selectedMonth] integerValue];
                
                NSInteger selectedDay = [pickerView selectedRowInComponent:2];
                
                NSInteger day = [self.days[selectedDay] integerValue];
                [self.delegate pickerViewDidStopWithYear:year
                                                   month:month
                                                     day:day];
