...
- (void)dismissConnectingAnimation {
    ...
    if (!didEnterCallBack) {
        [self showHUDInView:self.view justWithText:@"重置失败" disMissAfterDelay:1.5];
    }
}
            ...
            AQAlertView *alertView = [[AQAlertView alloc] initWithTitle:@""
                                                     ...
                                                     confirmButtonTitle:@"确定"
                                                         ...
                                                         confirmHandler:^(AQAlertView *alertView) {
                                                             ...
                                                             [NSTimer scheduledTimerWithTimeInterval:8 target:self selector:@selector(dismissConnectingAnimation) userInfo:nil repeats:NO];
                                                             
                                                             didEnterCallBack = NO;
                                                             
                                                             [[BTModel sharedInstance] deleteAllUserCompletion:^(id responseData) {
                                                                 
                                                                 didEnterCallBack = YES;
