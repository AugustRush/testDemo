...
{
    NSInteger numberOfRows = [_SKSTableViewDelegate tableView:tableView numberOfRowsInSection:section];
    NSInteger numberOfExpanded =  [[[self expandedIndexPaths] objectAtIndex:section] count];
    return numberOfRows + numberOfExpanded;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (![self.expandedIndexPaths[indexPath.section] containsObject:indexPath]) {
        ...
        SKSIndexPath *indexPathForSubrow = [self correspondingIndexPathForSubRowAtIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (![self.expandedIndexPaths[indexPath.section] containsObject:indexPath]) {
        ...
        SKSIndexPath *indexPathForSubrow = [self correspondingIndexPathForSubRowAtIndexPath:indexPath];
        [_SKSTableViewDelegate tableView:(SKSTableView *)tableView didSelectSubRowAtIndexPath:indexPathForSubrow];
...
{
    SKSIndexPath *sksIndexPath = [SKSIndexPath indexPathWithIndexPath:[self correspondingIndexPathForRowAtIndexPath:indexPath]];
    return [_SKSTableViewDelegate tableView:self numberOfSubRowsAtIndexPath:sksIndexPath];
}
...
- (NSIndexPath *)correspondingIndexPathForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    while (index < indexPath.row) {
        ...
        if (isExpanded) {
            SKSIndexPath *sksIndexPath = [SKSIndexPath indexPathWithIndexPath:[self correspondingIndexPathForRowAtIndexPath:tempIndexPath]];
            NSInteger numberOfExpandedRows = [_SKSTableViewDelegate tableView:self numberOfSubRowsAtIndexPath:sksIndexPath];
...
- (SKSIndexPath *)correspondingIndexPathForSubRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSInteger index = 0;
    NSInteger row = 0;
    NSInteger subrow = 0;
    ...
    return [SKSIndexPath indexPathForSubRow:subrow inRow:row inSection:indexPath.section];
}
...
@implementation SKSIndexPath

+ (SKSIndexPath *)indexPathForSubRow:(NSInteger)subrow inRow:(NSInteger)row inSection:(NSInteger)section {
    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:row inSection:section];
    SKSIndexPath *sksIndexPath = [[SKSIndexPath alloc] init];
    sksIndexPath.indexPath = indexPath;
    sksIndexPath.subRow = subrow;
    return sksIndexPath;
}

+ (SKSIndexPath *)indexPathWithIndexPath:(NSIndexPath *)indexPath {
    SKSIndexPath *sksIndexPath = [[SKSIndexPath alloc] init];
    sksIndexPath.indexPath = indexPath;
    return sksIndexPath;
}
