 ...
 *
 *
 *  @param handler 回调block，其中infos为字典，包含坑位，身高，年龄，性别
 */
+(void)getAllUserInfosInBodyScaleWithCompleteHandler:(void(^)(NSError *error,NSArray *infos))handler;

/**
 *  删除秤内某用户数据
 *
 *  @param location 用户的坑位 1 - 8
 *  @param handler  完成回调
 */
+(void)deleteUserMesureDataWithLocation:(NSUInteger)location completeHandler:(void(^)(NSError *error))handler;
 ...
 *  @param handler 回调
 */
+(void)resetBodyScaleWithCompleteHandler:(void(^)(NSError *error))handler;
 ...
 *  @param handler 回调
 */
+(void)getBodyScaleSoftVersionWithCompleteHandler:(void(^)(NSError *error,NSNumber *version))handler;
