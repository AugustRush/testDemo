...
- (void)dealloc
{
    ...
    //release user custom define
    if (errorCode) {
        [errorCode release];  errorCode = nil;
    }
    if (jasonItems) {
        [jasonItems release]; jasonItems = nil;
    }
        ...
        @autoreleasepool {
            if ([self isKindOfClass:[ASIFormDataRequest class]]) {
                ...
                if(responseString){
                    ...
                    self.jasonItems = [responseString objectFromJSONString];
                    
                    if (self.jasonItems) {
                        NSString *errCode = [self.jasonItems  objectForKey:@"errorCode"];
                        
                        if (NotNilAndNull(errCode))   self.errorCode = errCode;
                    }		
                }else{
